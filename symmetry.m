...
%         output = [max_rot_centre, sym_strength'];
    case{'mirror','mirror results'}
        ...
        for i=1:1%length(max_r) %min(3,length(max_r))
            ...
            [left_ind{i},right_ind{i},pts{i},pts_m{i}] = ...
                ...
                match_ind_m,r,ang,max_r(i),max_ang(i),grouping_thresh,ind{i});
        end

        output=display_output(im,keyVector,ang,sym_x,sym_y,ind,match_ind, match_ind_m,...
...
function output=display_output(im,keyVector,ang,sym_x,sym_y,... % first 5 parameters
...
%         print('-djpeg','results/butterfly_eg4.jpg');
        ...
        figure(6);
        output=display_output_6(im_dark,max_r,max_ang,ind,sym_x,sym_y,left_ind,right_ind,pts,pts_m,sym_strength);
        format_figure(im);
...
function [output]=display_output_6(im_dark,max_r,max_ang,ind,sym_x,sym_y,left_ind,right_ind,pts,pts_m,sym_strength);
...
%     if isempty(ind{i}), continue, end
    ...
    % axis.
    if abs(diff(unwrap([max_ang(i),pi]))) < (pi/16)
        Y = [min(sym_y(ind{i})) - tol : ...
            (max(sym_y(ind{i})) - min(sym_y(ind{i})))/300 : ...
            max(sym_y(ind{i})) + tol];
        X = ((-Y + centre(1))*sin(max_ang(i)) + max_r(i)) / cos(max_ang(i)) + centre(2);
    else
        X = [min(sym_x(ind{i})) - tol : ...
            (max(sym_x(ind{i})) - min(sym_x(ind{i})))/300 : ...
            max(sym_x(ind{i})) + tol];
